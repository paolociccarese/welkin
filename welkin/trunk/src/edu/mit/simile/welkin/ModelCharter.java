...
public class ModelCharter extends JComponent {
    ...
    ModelCharter(ModelWrapper model) {
        this.addMouseMotionListener(new MyMouseMotionListener());
        this.addMouseListener(new MyMouseListener());
        this.model = model;
    }

    void setGraph(ModelWrapper model) {
        this.model = model;
        analyze();
    }

    ModelWrapper getGraph() {
        return this.model;
    }
     ...
     */
    private int clustCoeff(Node node) {
        int neighbors = 0;
        int neighborEdges = 0;
        for (Iterator it = node.linkedObjectNodes.iterator(); it.hasNext();) {
            Edge e = (Edge) it.next();
    ...
    public void paintComponent(Graphics g) {
        ...
        for (Iterator it = inDegreeDistribution.keySet().iterator(); it.hasNext();) {
            ...
            if (count != null && degree.intValue() > 0) {
                float x = WIDTH * (float) (Math.log(degree.doubleValue())/Math.log((float) model.cache.nodes.size()));
        ...
        for (float f = 0.0f; f < LENGTH; f += 1.0f) {
            ...
            if (clustCoeffCount != null) {
                float w = HEIGHT * (float) clustCoeffCount.count / (float) model.cache.nodes.size();
